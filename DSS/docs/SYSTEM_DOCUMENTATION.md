...
- **API auth:** `api/index.php` requires `X-API-KEY` / `api_key` from `config/database.local.php` (`api_key`), compared with a timing-safe `hash_equals()`.
...
1. **`system_config` schema conflict** (RESOLVED) — `EmailService::createFromDbConfig()` now reads the real single-row `system_config` instead of a non-existent key/value table; SMTP config still comes from env/local config.
2. **Hardcoded API key** (RESOLVED) — moved to `config/database.local.php` (`api_key`) and compared with `hash_equals()` in `api/index.php`.
3. **Hardcoded term** (RESOLVED) — controllers now read the active term via `getActiveAcademicTerm($pdo)` (year from `system_config.active_academic_year` + " Semester 1").
4. **Duplicate activity logging** (RESOLVED) — `action_add_project.php` logs CREATE once.
...
6. **`ALTER TABLE` migrations at runtime** (INTENTIONALLY LEFT) — login_action adds 2FA cols; optimization.php adds actual_budget/variance/actual_volunteer_hours. Additive/safe; deferred per deployment constraints.
