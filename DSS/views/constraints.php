...
<?php else: ?>
    <div class="bento-card bento-span-full">
        <div class="bento-card-header"><i class="material-icons">settings</i> <?= __('term_budget_settings') ?></div>
    ...
    <div class="bento-card bento-span-full">
        <div class="bento-card-header"><i class="material-icons">history</i> <?= __('previous_term_settings') ?></div>
    ...
    <div class="bento-card bento-span-full">
        <div class="bento-card-header"><i class="material-icons">account_balance_wallet</i> <?= __('budget_watch') ?></div>
        ...
        <div class="bento-grid" style="margin-top: 1.5rem;">
            <div class="bento-card" style="text-align: center;">
                <div class="bento-card-header"><?= __('total_budget') ?></div>
        ...
        <p style="margin-top: 0.5rem; font-size: 0.875rem; color: <?= $budgetUtilization > 90 ? '#dc2626' : '#6b7280' ?>;">
            <?= number_format($budgetUtilization, 1) ?>% used
                <?= $budgetUtilization > 90 ? __('budget_almost_finished') : '' ?>
                <?= $budgetUtilization > 70 && $budgetUtilization <= 90 ? __('budget_running_low') : '' ?>
        </p>
    </div>

    <?php if (in_array($sessionRole, ['Financial Secretary', 'Executive Board'])): ?>
    <div class="bento-card bento-span-full">
        <div class="bento-card-header"><i class="material-icons">tune</i> <?= __('set_term_budget_scoring') ?></div>
        <p class="bento-subtext">
            ...
            <form method="post" action="/dss/controllers/action_set_constraints.php">
                ...
                <div class="row">
                    ...
                    <div class="input-field col s12 m6">
                        <input
                            ...
                            required
                        >
                        <label for="max_available_budget"><?= __('total_budget_gHS') ?></label>
                    </div>
                    <div class="input-field col s12 m6">
                        <input
                            type="number"
                    ...
                    <div class="importance-card col s12 m6">
                        <div class="importance-group-label"><?= __('reach_priority') ?></div>
                ...
                <h5 style="margin: 2rem 0 1rem; color: #025928;"><?= __('umat_project_importance_ratings') ?></h5>
                ...
                <button type="submit" class="btn green"><i class="material-icons left">save</i><?= __('save_term_settings') ?></button>
            </form>
        </div>
    </div>
    <?php endif; ?>
    ...
    <div class="bento-card bento-span-full">
        <div class="bento-card-header"><i class="material-icons">how_to_vote</i> Team Voting — Share Your Preferences</div>
                <p class="bento-subtext"><?= __('vote_priority_desc') ?></p>

            <form method="post" action="/dss/controllers/model_management/action_gdss_vote.php" style="margin-bottom: 1.5rem;">
                <?= $csrfField ?>
                <div class="row">
                    <div class="input-field col s12">
                        <input type="text" name="academic_term" value="<?= htmlspecialchars(getActiveAcademicTerm($pdo), ENT_QUOTES, 'UTF-8') ?>" required>
                    ...
                    <div class="input-field col s12 m6">
                        ...
                        <label><?= __('your_reach_preference') ?></label>
                    </div>
                    <div class="input-field col s12 m6">
                ...
                <button type="submit" class="btn blue"><i class="material-icons left">how_to_vote</i><?= __('submit_my_vote') ?></button>
            </form>
            ...
            <?php if ($sessionRole === 'Executive Board'): ?>
                <div class="card-panel grey lighten-4">
                    ...
                    <form method="post" action="/dss/controllers/action_gdss_finalize.php">
                        <?= $csrfField ?>
                        <input type="hidden" name="academic_term" value="<?= htmlspecialchars(getActiveAcademicTerm($pdo), ENT_QUOTES, 'UTF-8') ?>">
    ...
    <div class="bento-card bento-span-full">
        <div class="bento-card-header"><i class="material-icons">how_to_vote</i> <?= __('team_votes') ?></div>
        <p class="bento-subtext"><?= __('see_all_votes') ?></p>
        <?php if ($gdssVotes === []): ?>
            <div class="center-align" style="padding: 3rem 1rem;">
                <i class="material-icons large grey-text text-lighten-2">how_to_vote</i>
                <h5 class="grey-text"><?= __('no_votes_yet') ?></h5>
                <p class="grey-text"><?= __('no_team_votes') ?></p>
            </div>
        <?php else: ?>
            <div class="responsive-table-shell" style="margin-top: 1.5rem;">
            <table class="striped highlight responsive-table">
                    <thead>
                        <tr>
                            <th><?= __('term') ?></th>
                            <th><?= __('user') ?></th>
                            <th class="text-right"><?= __('reach') ?></th>
                            <th class="text-right"><?= __('speed') ?></th>
    ...
    <div class="bento-card bento-span-full">
        <div class="bento-card-header"><i class="material-icons">tune</i> <?= __('criteria_weights_history') ?></div>
        <p class="bento-subtext"><?= __('criteria_history_desc') ?></p>
        <?php if ($criteriaHistory === []): ?>
            <div class="center-align" style="padding: 3rem 1rem;">
                <i class="material-icons large grey-text text-lighten-2">tune</i>
                <h5 class="grey-text"><?= __('no_criteria_history') ?></h5>
                <p class="grey-text"><?= __('no_criteria_weights_saved') ?></p>
            </div>
        <?php else: ?>
            <div class="responsive-table-shell" style="margin-top: 1.5rem;">
            <table class="striped highlight responsive-table">
