    ...
    <script>
        ...
        document.getElementById('sandboxForm')?.addEventListener('submit', function(e) {
            ...
            fetch('/dss/controllers/model_management/action_run_sandbox.php', {
                method: 'POST',
                body: formData,
        ...
        document.getElementById('goalSeekingForm')?.addEventListener('submit', function(e) {
            ...
            fetch('/dss/controllers/model_management/action_run_goalseeking.php', {
                method: 'POST',
        ...
        document.getElementById('runAiBtn')?.addEventListener('click', function(e) {
            ...
            fetch('/dss/controllers/model_management/action_run_expert_system.php', {
                method: 'POST',
                body: formData,
                signal: controller.signal
            })
        ...
        (function() {
            ...
            setInterval(function() {
                fetch('/dss/controllers/action_poll_updates.php')
                    .then(r => r.json())
                    .then(data => {
                        if (data.status !== 'success') return;
                        const badge = document.getElementById('notificationBadge');
                        if (data.notifications && data.notifications.unread_count > 0) {
                            if (!badge) {
                                ...
                                if (bell) {
                                    ...
                                    b.textContent = data.notifications.unread_count > 99 ? '99+' : data.notifications.unread_count;
                                    bell.appendChild(b);
                                }
                            } else {
                                badge.textContent = data.notifications.unread_count > 99 ? '99+' : data.notifications.unread_count;
                            }
                        } else if (badge) {
                            badge.remove();
                        }
