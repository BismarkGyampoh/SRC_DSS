...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    header('Location: /dss/views/sandbox.php');
    exit;
}
...
$simReachRaw = (int) ($_POST['sim_reach_weight'] ?? 0);
$simSpeedRaw = (int) ($_POST['sim_speed_weight'] ?? 0);
$simWeightTotal = $simReachRaw + $simSpeedRaw;

if ($simWeightTotal <= 0) {
    $_SESSION['flash_message'] = 'Please choose an importance level for both Reach and Speed.';
    header('Location: /dss/views/sandbox.php');
    exit;
}

$simReachWt = round($simReachRaw / $simWeightTotal, 4);
$simSpeedWt = round($simSpeedRaw / $simWeightTotal, 4);
